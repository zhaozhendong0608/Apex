...
为解决长期开发中大模型上下文膨胀 (BUG-003) 缺陷，工作流建立了三层立体的记忆与掌控模型：

1. **⚡ 第一层：动态极简快照名片 (`.ai/snapshot.md`)**
...
2. **🧩 第二层：宏观业务大模块地图 (`docs/modules/`)**
   - **特点**：记载项目的业务大模块分类（用户中心、支付、排放源配置）。
   - **使用**：输入 **`1+ 模块名`** 开启新大功能时增量追加，防范 AI“只见树木不见森林”。
3. **📦 第三层：微观原子任务流水账 (`.ai/handover.md` & `status.md`)**
